...
def helper():
        ...
        print("\tCtrl+Z: Quits WeVulScanner.")
...
class Spinner:
    ...
    def spinner_task(self):
        ...
        try:
            ...
            print("\n\t"+ bcolors.BG_ERR_TXT+"WeVulScanner received a series of Ctrl+C hits. Quitting..." +bcolors.ENDC)
            sys.exit(1)
...
if args_namespace.help or (not args_namespace.update \
    ...
    logo()
    print("WeVulScanner is updating....Please wait.\n")
    spinner.start()
    ...
    if oldversion_hash == newversion_hash :
        clear()
        print("\t"+ bcolors.OKBLUE +"You already have the latest version of WeVulScanner." + bcolors.ENDC)
    else:
        clear()
        print("\t"+ bcolors.OKGREEN +"WeVulScanner successfully updated to the latest version." +bcolors.ENDC)
    spinner.stop()
    sys.exit(1)
    ...
    while (rs_avail_tools < len(tools_precheck)):
        ...
        try:
            ...
            print("\t"+bcolors.BG_ERR_TXT+"WeVulScanner was terminated abruptly..."+bcolors.ENDC)
            sys.exit(1)
    ...
    if len(unavail_tools_names) == 0:
        print("\t"+bcolors.OKGREEN+"All Scanning Tools are available. Complete vulnerability checks will be performed by WeVulScanner."+bcolors.ENDC)
    else:
        print("\t"+bcolors.WARNING+"Some of these tools "+bcolors.BADFAIL+str(unavail_tools_names)+bcolors.ENDC+bcolors.WARNING+" are unavailable or will be skipped. WeVulScanner will still perform the rest of the tests. Install these tools to fully utilize the functionality of WeVulScanner."+bcolors.ENDC)
    ...
    while(tool < len(tool_names)):
        ...
        if runTest == 1:
                ...
                print("\n"+bcolors.WARNING + "\tTest Skipped. Performing Next. Press Ctrl+Z to Quit WeVulScanner.\n" + bcolors.ENDC)
                rs_skipped_checks = rs_skipped_checks + 1
    ...
    if len(rs_vul_list)==0:
        ...
        with open(vulreport, "a") as report:
            ...
            print("\tComplete Vulnerability Report for "+bcolors.OKBLUE+target+bcolors.ENDC+" named "+bcolors.OKGREEN+vulreport+bcolors.ENDC+" is available under the same directory WeVulScanner resides.")
